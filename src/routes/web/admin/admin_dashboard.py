...
async def _collect_runtime_metrics(duration_sec: int) -> dict:
    ...
    try:
        ...
        endpoint_failure_counts = await redis_client.hgetall("metrics:endpoint_failures")
        endpoint_last_errors = await redis_client.hgetall("metrics:endpoint_last_error")
...
async def admin_dashboard(request: Request, duration_sec: int = Query(300)):
    ...
    recent_users_cursor = users_collection.find({}).sort("created_at_timestamp", -1).limit(8)
