...
from src.utils.admin_auth import mask_sensitive_fields
...
@router.get("/credentials", name="admin_credentials", include_in_schema=False)
async def admin_credentials(
    ...
    credentials_raw = await cursor.to_list(length=PAGE_SIZE)
    credentials = [mask_sensitive_fields(dict(c)) for c in credentials_raw]
