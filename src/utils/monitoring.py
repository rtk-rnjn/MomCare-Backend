...
class MonitoringHandler:
    ...
    def log_request(self, method: str, path: str, status_code: int, duration_ms: float, user_agent: str = "", ip_address: str = ""):
        ...
        cursor = self.conn.cursor()
        cursor.execute(
            "INSERT INTO api_requests (timestamp, method, path, status_code, duration_ms, user_agent, ip_address) VALUES (?, ?, ?, ?, ?, ?, ?)",  # noqa
    ...
    def get_stats(self, hours: int = 24):
        ...
        # Top endpoints
        cursor.execute(
            "SELECT path, COUNT(*) as count FROM api_requests WHERE timestamp > ? GROUP BY path ORDER BY count DESC LIMIT 10",
            (cutoff_time,),
        )
        ...
        # Recent errors
        cursor.execute(
            "SELECT timestamp, method, path, error_type, error_message FROM api_errors WHERE timestamp > ? ORDER BY timestamp DESC LIMIT 20",  # noqa
            (cutoff_time,),
        )
        recent_errors = [
            {
